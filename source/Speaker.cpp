...
static const uint8_t beep_1[] = {
    0x80,0x80,0x80,0x7F,0x7F,0x80,0x80,0x7F,0x7F,0x80,0x80,0x80,0x80,0x7F,0x7F,0x7F,0x7F,0x7F,0x7F};
...
say_beep_1()
{
    ...
    int x3 = 0;
    while (x0 < 40) {
        sampleSource->play(middleE, sizeof(middleE));
        x0++;
    }
    while (x1 < 60) {
        sampleSource->play(middleC, sizeof(middleC));
        x1++;
    }
    while (x2 < 80) {
        sampleSource->play(middleC, sizeof(middleC));
    ...
    while (x3 < 20) {
        sampleSource->play(middleD, sizeof(middleD));
        x3++;
    }
}
